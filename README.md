...
GUI ไม่มีช่องตั้งจำนวนเธรดแล้ว engine เริ่มที่ **GenID 2 / Login·Level3·Stage·Quest 32** เธรดต่อ IP
(`autoscale.START_BY_MODE`) แล้วปรับเองทุก 10 วิ เป้าคือไอดี/นาทีสูงสุด:

- **เพิ่มทีละ 50%** ตราบที่ req/s ที่ IP นั้นยิงออกได้จริงยังขึ้นตาม (ขึ้นไม่ถึง 20% ของสัดส่วนที่เพิ่ม = คอขวดอยู่ที่อื่น
  เช่น CPU → ถอยกลับเลขเดิม แล้วลองใหม่ทุก 60 วิ)
- **ตัดลง** เมื่อเธรดเกิน 35% ยืนรอถัง req/s (`apirps`) หรือโควตา mint ของ IP — เพิ่มไปก็ได้แค่คิวยาว
  (GenID จึงเหลือไม่กี่เธรด เพราะเพดานคือ mint 2 ครั้ง/นาที/IP ไม่ใช่จำนวนเธรด)
- **ถอย 25%** เมื่อเจอ HTTP 429/503 ของ nginx เกิน 2% ของ request ในรอบนั้น
- เพดาน 256 เธรดต่อ IP และ `maxthreads` รวมทุก IP; เธรดที่เกินเป้าออกเองหลังจบไอดีที่ถืออยู่ (ไม่ตัดกลางไอดี)
- กลับไปใช้เลขคงที่แบบเดิมได้ด้วย `autothreads = False` ใน `[settings]` (แล้ว `threadcount` = เธรดรวม)

จำลองด้วยเธรดจริง + ถังจริง (`bot/tests/bench_autoscale.py`, window 3 วิ): ถัง 80 req/s ที่อิ่มจริง ~32 เธรด → ไปหยุดที่
42 เธรด ได้ ~440 จากเพดาน 480 ไอดี/นาที; เพดาน "CPU" 24 ช่องไม่มีถัง → ถอยกลับ 24–36 เธรด; โควตา mint → เหลือ 2 เธรด
ได้เต็มโควตา — **ยังไม่ได้วัดกับเซิร์ฟเวอร์จริง**

แผงซ้ายของ GUI: เลขเธรดที่รันอยู่ + สถานะ autoscaler (กำลังเพิ่ม / เต็มเพดาน IP / เพิ่มแล้วไม่เร็วขึ้น / โดน 429 ถอย),
