...
### จำนวนเธรดอัตโนมัติ + ตารางบัญชีที่กำลังทำ (`bot/engine/autoscale.py`, 2026-09-24)

GUI ไม่มีช่องตั้งจำนวนเธรดแล้ว engine เริ่มที่ **GenID 2 / โหมดอื่น 32** เธรดต่อ IP (`autoscale.START_BY_MODE`)
แล้วปรับเองทุก 5 วิ สองคันโยก เป้าคือไอดี/นาทีสูงสุด:

- **งบ req/s ต่อ IP (AIMD, 2026-09-25):** เริ่มที่ `apirps` เธรดยืนรอถังของเราเอง (≥10%) และ 429/503 ≤ 0.5% → ขยายงบ 15%;
  429/503 > 2% → หดงบ 20% แล้วห้ามขยาย 30 วิ; หลังเคยโดน 429 ขยายใกล้จุดนั้นทีละ 3% เพดาน `apirpsmax` (ค่าปริยาย 400,
  ใส่เท่ากับ `apirps` = งบคงที่แบบเดิม) — รันจริงก่อนหน้านี้ค้าง 108 เธรด ~250 ไอดี/นาทีเพราะชนงบ 80 req/s ของเราเอง
  ทั้งที่ nginx ตอบ 429 แค่ 0.4% ที่ 106 req/s (วัด 2026-09-22)
- **จำนวนเธรด:** เพิ่มทีละ 50% ตราบที่ req/s ที่ใช้ได้จริงยังขึ้นตาม (ขึ้นไม่ถึง 20% ของสัดส่วนที่เพิ่ม = คอขวดอยู่ที่อื่น
  เช่น CPU → ถอยกลับ ลองใหม่ทุก 60 วิ); ต่อไม่ติด/หลุดซ้ำ > 2% → ถอย 25%; หลังเคยชนขอบ เข้าหาขอบเดิมทีละ 10%
  (เดิมโตกลับ x1.5 ชนซ้ำทุกนาที: 156 → 96 → 141 → 89, ไอดี/นาทีตก ~250 → ~190)
- **ตัดเธรด** เมื่อยืนรอโควตา mint (GenID เหลือไม่กี่เธรด เพดานคือ mint 2 ครั้ง/นาที/IP) หรือรอถังที่ขยายต่อไม่ได้แล้ว
- เพดาน 512 เธรดต่อ IP และ `maxthreads` รวมทุก IP; เธรดที่เกินเป้าออกเองหลังจบไอดีที่ถืออยู่ (ไม่ตัดกลางไอดี)
- กลับไปใช้เลขคงที่แบบเดิมได้ด้วย `autothreads = False` ใน `[settings]` (แล้ว `threadcount` = เธรดรวม, งบ = `apirps`)
- ยอด/สถานะ autoscaler ทุก 2 วิของรันล่าสุดอยู่ใน `bot/src/log/engine-stats.jsonl` (เขียนทับทุกรัน) ไว้ย้อนดูตอนจูน

จำลองด้วยเธรดจริง + ถังจริง (`bot/tests/bench_autoscale.py`, 10 request/ไอดี, ช่วงนิ่งหลังนาทีแรก):

| สถานการณ์ | ผล | เพดานทางทฤษฎี |
|---|---|---|
| `server`: งบเริ่ม 80, เซิร์ฟเวอร์รับ 150 req/s (เกินตอบ 429) | งบขยาย 80→161 โดน 429 แล้วเลียบขอบ 139–158, ยิงจริงเฉลี่ย 146 req/s, **876 ไอดี/นาที** | 900 |
| `bucket`: งบตายตัว 80 (`apirpsmax = apirps`) | 48 เธรด, 481 ไอดี/นาที | 480 |
| `cpu`: ไม่มีถัง เครื่องรับงานพร้อมกันได้ 24 ช่อง | 32 เธรด (เพิ่มแล้วไม่คุ้มเลยถอย), 359 ไอดี/นาที | 360 |
| `genid`: โควตา mint 2 ครั้ง/6 วิ | 2 เธรด, ~18 ไอดี/นาที | 20 |

**ยังไม่ได้วัดกับเซิร์ฟเวอร์จริง** — ขอบจริงของ nginx ต่อ IP (429 เกิน 2%) อยู่แถว ~140 req/s (วัด 2026-09-22)

แผงซ้ายของ GUI: เลขเธรดที่รันอยู่ + สถานะ autoscaler (กำลังเพิ่ม / ขยายงบ req/s / โดน 429 ลดงบ / เต็มเพดาน / ...), ยิงจริง/งบ req/s,
ยอด สำเร็จ·ล้ม·เหลือ·**ไอดี/นาที (60 วิล่าสุด)** และตารางไอดีที่กำลังทำ (ไอดี / ขั้น / เวลา / log สด เช่น `st045/150 Lv12`)
engine ส่งตารางทั้งก้อนเป็นแถว JSONL `{"t":"active","rows":[...]}` ทุกวินาที ขั้นของแต่ละไอดีมาจาก `s.mark()` ใน `flows.py`

**ปุ่ม Stop หยุดทุกอย่างภายในไม่กี่วินาที** (เดิมปล่อยไอดีที่ค้างทำจนจบ — Stage/Quest ~9 นาทีต่อไอดี กด Stop แล้วยังวิ่งต่อ):
GUI เขียน `src/log/stop.flag` → engine (เช็คทุก 0.2 วิ) สั่ง `lane.stop()` → request ถัดไปของทุกไอดีโยน
`EngineStopped` (BaseException ให้ `except Exception` ใน flows ดักไม่ได้) รอ retry ก็ตื่นทันที (`lane.nap`) →
ไฟล์ของไอดีที่ถูกตัดคืนเข้า `input/` โฟลเดอร์ย่อยเดิม (`WorkQueue.release`) ไม่ถูกนับเป็น done/fail → engine เลิกรอเธรดที่
ยังหลับ (รอหัวใจ/โควตา) หลัง 3 วิ ส่งยอดสุดท้าย (`stopped` = จำนวนที่หยุดกลางทาง) แล้ว `os._exit` —
GUI บังคับ terminate ถ้าเกิน 10 วิ (`STOP_GRACE_SECONDS`) และ Stop ทำงานไม่ว่า dropdown จะเลือกโหมดไหนอยู่
ทดสอบกับ engine จริง (ไฟล์ปลอม 5 ใบ ไม่ยิงเน็ต): โปรเซสออกภายใน 0.1 วิหลังเขียน stop.flag ไฟล์กลับ `input/` ครบ 5 ใบ

**รันต่อเนื่อง ไม่หยุดเองกลางทาง** (แก้ 2026-09-25 หลังรัน Login Lv3 หยุดเองที่ ~160 เธรด ด้วย
`lane direct: dead` → `every proxy is down - stopping`):
- ต้นเหตุ: `rewards.survey` แตก thread pool 7 ตัวต่อไอดี — lane เป็น thread-local เธรดลูกจึงไม่มี lane แล้ว
  `rangers_api` ถอยไปใช้ถังแบบไฟล์ล็อก (`.ratelimit/bucket-direct-*.txt`) ที่ ~1,000 เธรดแย่งล็อกไฟล์จนค้าง 10 วิ
  (`engine.err`) ไอดีเสร็จเหลือ 2-3 ใบ/10 วิ request ค้างจน timeout แล้ว lane ถูกตัดสินว่าตาย — ตอนนี้ใน engine
  `survey` ถามทีละแหล่งบนเธรดเดิม (ใช้ถัง/proxy/Stop ของ lane) autoscaler จึงเห็น request พวกนี้ด้วย
- lane `direct` (ไม่ได้ตั้ง proxy) ไม่มีวันตาย; lane proxy ตายเมื่อพัง ≥3 ครั้งติดกัน **และ** ไม่มีคำตอบสำเร็จเลย 20 วิ;
  socket keep-alive ที่เซิร์ฟเวอร์ปิดทิ้ง (พังครั้งแรกแล้วต่อใหม่) ไม่นับ
- proxy ล่มหมดทุกตัว: ไม่หยุดรันแล้ว รอ 30 วิ (`ALL_DOWN_RETRY`) แล้วลองทุกตัวใหม่ (ยังไม่ยิงออก IP ของเครื่องเอง)
- ต่อไม่ติด/หลุดซ้ำนับเป็นแรงต้านให้ autoscaler ถอยเธรด (รวมกับ 429/503)
- `print()` ของ tools/ ใน engine ไปลง devnull — เดิมปนกับช่อง JSONL ทำให้แถวไอดีบางแถว parse ไม่ออกแล้วหายจากยอด
