...
รัน: PYTHONUTF8=1 python bot/tests/bench_autoscale.py bucket|cpu|genid [วินาที]
- bucket: ผนังคือถัง 80 req/s ของเราเอง (โหมด Login)
...
autoscale.WINDOW = 3.0
autoscale.PROBE_AFTER = 15.0
pool_mod.STAT_EVERY = 3.0

RPS, LAT, REQ = 80, 0.4, 10          # อิ่มที่ ~32 เธรด
...
class Tee(io.StringIO):
    def write(self, text):
        for line in text.splitlines():
            row = json.loads(line)
            if row["t"] == "stat":
                print("%5.0fs threads=%-4s target=%-4s scale=%-8s rpm=%-7s done=%s" % (
                    time.time() - t0, row.get("threads"), row.get("target"), row.get("scale"),
                    row.get("rpm"), row.get("done")), flush=True)
        return len(text)
