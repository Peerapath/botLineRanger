...
| pass 2 และ 3 สำรวจแค่ `giftbox` ไม่ใช่ครบ 7 แหล่ง — ระบบอื่นจ่ายของ *เข้า* giftbox ไม่ได้จ่ายเข้าหากัน | −12 | 0 |
...
ค่าเฉลี่ยใหม่จึงอยู่ราว **16** (จาก 26.4) = **ลดลง ~39%**

ตั้งค่าใหม่ใน `[settings]`: `rewardpasses = 3` (เท่าเดิม แต่เดิมเขียนตายในโค้ด ตอนนี้ตั้งได้)

> **แก้ไข 2026-09-24 หลังรีวิว Task 5:** ฉบับแรกของ spec นี้ตัด pass 3 ทิ้งแล้วตั้งค่าเริ่มต้น
> เป็น 2 โดยให้เหตุผลว่า "ระบบอื่นจ่ายของเข้า giftbox ไม่ได้จ่ายเข้าหากัน" — เหตุผลนั้นถูกสำหรับ
> ระบบอื่นทั้งหก แต่**ไม่ได้พูดถึงว่า giftbox จ่ายเข้าตัวเองได้ไหม** และ `claim_giftbox` มีลูป
> เก็บกล่อง MINI_GACHA ที่ `receive/all` กวาดไม่ได้ ซึ่งเป็นหลักฐานว่าของในกล่องไม่ได้ถูกเก็บ
> ด้วยวิธีเดียวกันทั้งหมด ถ้าการเปิดของชิ้นหนึ่งวางของชิ้นใหม่กลับลงกล่อง 2 pass จะกวาดไม่หมด
> และของหายเงียบ ๆ โดยไม่มีใครรู้ — โค้ดในรีโปนี้ยืนยันไม่ได้ทั้งสองทาง
>
> ราคาของการเก็บ pass 3 ไว้คือ **1 request** (`GET /giftbox/list` ที่ไม่เจออะไรแล้ว break) เพราะ
> `REFRESH_SOURCES` ตัด 6 ใน 7 แหล่งออกจากทุก pass หลังแรกไปแล้ว — ส่วนลดตัวจริงอยู่ตรงนั้น
> ไม่ใช่ที่จำนวน pass ข้อผูกพันของ task นี้คือ "ไม่ทำให้เก็บของได้น้อยลง" จึงชนะการประหยัด
> 1 request

**ผลต่อความเร็ว:** งบ 90 req/s ต่อ IP (เผื่อขอบจากเพดาน nginx ~100) ÷ 16 request =
**5.6 บัญชี/วิ = ~337/นาที ต่อ 1 IP** เทียบกับ 181/นาที ตอนนี้ = **1.9×**
...
- จำนวนเธรดต่อ lane มาจากกฎของ Little: `concurrency = latency × rate` ที่ 16 request/บัญชี
  latency ราว 24 วิ (วันนี้ p50 = 40 วิ ที่ 26 request) × 5.6 บัญชี/วิ ≈ **135 เธรด/IP**
...
| request/บัญชี (เฉลี่ย) | 26.4 | ~16 | นับใน `test_session.py` + `bench_engine.py` |
| บัญชี/นาที @ 1 IP | 181 | ~337 | รอบจริงกับ `input/` ชุดที่แยกไว้ |
...
1. **nginx ฝั่งเกมจำกัด ~100 req/s ต่อ IP** → เพดานคือ ~6.3 บัญชี/วิ/IP ที่ 16 request/บัญชี
   ทางเดียวที่จะเกินคือเพิ่ม IP · ตัวเลขนี้เป็นเพดานของ*ปลายทาง* การเขียนโค้ดดีขึ้นไม่ช่วย
2. **per-account min-gap ~300 ms** (ตอบเป็น HTTP 400 + `errorCode 429`) → 16 request =
   อย่างน้อย 4.8 วินาทีต่อบัญชี ต่อให้เน็ตเร็วเป็นศูนย์ ซึ่งแปลว่าการลด request ต่ำกว่านี้
...
6. **การย้ายไฟล์ของคิวเองเป็นเพดานที่ ~27-29 proxy** (วัด 2026-09-24 บนเครื่องนี้)

   `bench_engine.py` ที่ตัดเน็ตออกหมดวัดได้ **~150-165 บัญชี/วิ** ไม่ใช่หลักพันตามที่คาด
   สาเหตุไม่ใช่ GIL ไม่ใช่ lock และไม่ใช่โค้ดของเรา — ทำซ้ำได้ด้วย `os.replace()`
   เปล่า ๆ ที่ ~86-88/วิ คือ **เพดานของ Windows เองเรื่อง rename ข้ามโฟลเดอร์พร้อมกัน**

   ที่เป้า 5.6 บัญชี/วิ/IP แต่ละบัญชีกิน 2 rename (claim + finish) → คิวรับได้ราว
   **27-29 proxy** ก่อนกลายเป็นคอขวดแทนเน็ต ซึ่ง**อยู่ในช่วง 5-50 ที่ตั้งใจจะใช้** ไม่ใช่เรื่องไกลตัว

   รายงานฉบับแรกสรุปว่า "ไม่เกี่ยวกับ production" โดยคิดจาก 128 บัญชีพร้อมกัน — แต่ 128
   คือเพดานแรมของสถาปัตยกรรม**เก่า**ที่การรื้อนี้มีไว้เพื่อเอาออก โจทย์จึงผิด reviewer จับได้

   **ที่ต้องทำก่อนเชื่อเลขนี้:** เพดานนี้อาจเป็นของเครื่องนี้เอง (สงสัย Defender real-time
   scanning) ต้องวัดซ้ำบนเครื่องที่จะรันจริง และลองยกเว้นโฟลเดอร์บอทออกจาก real-time scan
...
- `rewardpasses` ตั้งค่าเริ่มต้นเป็น 3 เท่าเดิม — ดูกล่องแก้ไขท้ายข้อ 5
